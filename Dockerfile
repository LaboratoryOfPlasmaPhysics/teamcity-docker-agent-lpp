...
RUN sudo add-apt-repository -y ppa:deadsnakes/ppa && sudo apt update -y && sudo apt upgrade -y && \
	sudo apt install -y libc6-i386 python3-virtualenv virtualenv python3-coverage python-coverage \
	python3-coverage-test-runner python-unittest2 python3-unittest2 python3-venv python3.6-venv \
        python3.7-venv python3.8-venv python3.9-venv python3-pip tox && \
